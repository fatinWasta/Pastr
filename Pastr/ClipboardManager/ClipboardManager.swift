...
class ClipboardManager: ObservableObject {
    ...
    private func checkForChanges() {
        ...
        // Check if the session history is empty before adding a new item.
        ...
        // If the history was empty, this is the first new item. Reset the session timer.
        if historyWasEmpty {
            beginSessionLoop()
        }
    }
    ...
    private func beginSessionLoop() {
        sessionManagementTask?.cancel()
        sessionManagementTask = Task {
            while !Task.isCancelled {
                // 1. Start a new session by setting the end date.
                sessionEndDate = Date().addingTimeInterval(sessionDuration)
                
                // 2. Wait for the session duration to pass.
                do {
                    try await Task.sleep(for: .seconds(sessionDuration))
                } catch {
                    // If the task is cancelled (e.g., on deinit or reset), just exit the loop.
                    return
                }
                
                // 3. When the timer finishes, notify and clear the unpinned items.
                sessionDidEnd.send()
                clearSession()
            }
        }
...
struct ContentAnalyzer {
    ...
    private static func analyzeToken(_ string: String) -> (ContentType, String)? {
        // --- 1. JWT (JSON Web Token) Check ---
        // Very specific pattern: three parts separated by dots, first part is Base64Url-encoded JSON.
        ...
        // --- 4. Generic Heuristic Checks for other tokens ---
        // A common characteristic is no whitespace and a minimum length.
        ...
        // Final fallback: check for a high density of alphanumeric characters.
...
private struct SwiftCodeStrategy: CodeAnalysisStrategy {
    private let indicators = ["import SwiftUI", "import Foundation", "struct ", "enum ", "@State", "func "]
